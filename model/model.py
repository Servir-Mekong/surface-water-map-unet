...
def decoder_block(input_tensor, concat_tensor=None, n_filters=512, n_convs=2, i=0, rate=0.2,
    ...
    # Neighbour Neighbour upsampling
    up = layers.UpSampling2D(interpolation='nearest', name=f'{name_prefix}{i}_upsamp')(deconv)
    # 2x2 stride 2
    ...
    # nearest neighbour resize convulation
    # up = layers.UpSampling2D(size=(2, 2), interpolation='nearest', name=f'{name_prefix}{i}_upsamp')(deconv)
    # up = layers.Conv2D(filters=n_filters, kernel_size=(3, 3), strides=(1, 1), padding='same', name=f'{name_prefix}{i}_resize_conv')(up)
    return up
